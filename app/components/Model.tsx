...
import React, { useRef, useEffect, forwardRef, RefObject, useMemo } from 'react'
...
export const Model = forwardRef<Group, ModelProps>(({ isAnimated = true, onConnectionPointsUpdate, isMobile = false, ...props }, ref) => {
  ...
  const chromeMaterial = useMemo(() => new MeshStandardMaterial({
    ...
    transparent: true,
    opacity: isMobile ? 0.4 : 0.05, // Plus opaque sur mobile car pas d'Edges
  ...
  useFrame((state) => {
    ...
    if (chromeMaterial && isAnimated && !isMobile) {
  ...
  return (
    <group ref={ref || group} {...props} dispose={null}>
      <group name="Scene">
        ...
        >
          {/* Les Edges sont très coûteux en draw calls, désactivés sur mobile */}
          {!isMobile && (
            <Edges
              geometry={(nodes.DNA_logo_3d as Mesh).geometry}
